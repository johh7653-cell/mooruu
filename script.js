...
document.querySelectorAll('a[href]').forEach((link) => {
  link.addEventListener("click", (event) => {
    ...
    if (href.startsWith("http") || href.startsWith("#") || link.target) return;
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;

    event.preventDefault();
    const rect = link.getBoundingClientRect();
    const transitionPanel = link.cloneNode(true);
    const pageFade = document.createElement("div");

    transitionPanel.classList.add("route-panel", "foregrounded");
    transitionPanel.style.setProperty("--route-left", `${rect.left}px`);
    transitionPanel.style.setProperty("--route-top", `${rect.top}px`);
    transitionPanel.style.setProperty("--route-width", `${rect.width}px`);
    transitionPanel.style.setProperty("--route-height", `${rect.height}px`);
    pageFade.className = "route-fade";

    document.body.append(pageFade, transitionPanel);
    document.body.classList.add("route-leaving");

    requestAnimationFrame(() => {
      transitionPanel.classList.add("expand");
